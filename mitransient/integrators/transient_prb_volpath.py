...
class TransientPRBVolpathIntegrator(TransientRBIntegrator):
            ...
            with dr.resume_grad(when=not is_primal):
                ...
                if self.handle_null_scattering:
                    ...
                    weight[act_null_scatter] *= dr.select(dr.neq(dr.detach(1 - scatter_prob), 0.0),
                                                        mei.sigma_n / dr.detach(1 - scatter_prob),
                                                        0.0)
                else:
                ...
                weight[act_medium_scatter] *= dr.select(dr.neq(dr.detach(scatter_prob), 0.0),
                                                        mei.sigma_s / dr.detach(scatter_prob),
                                                        0.0)
                ...
                distance = distance + dr.select(si.is_valid() & ~mei.is_valid(), si.t, 0.0) * η
    ...
    def sample_emitter(self, ref_interaction, scene, sampler, medium, channel,
        ...
        while loop(active):
            ...
            transmittance *= dr.select(dr.isfinite(dr.detach(tr_multiplier)), dr.detach(tr_multiplier), 0.0)
