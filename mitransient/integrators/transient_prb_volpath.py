...
class TransientPRBVolpathIntegrator(TransientADIntegrator):
    ...
    def sample(self,
        ...
        # depth_data = mi.Spectrum(self.firstSurface(scene, ray, active))
        # depth_data.y = 0.0
        # depth_data.z = 0.0
        # return depth_data
        ...
        if mi.is_rgb:  # Sample a color channel to sample free-flight distances
            ...
            channel = dr.minimum(
                n_channels * sampler.next_1d(active), n_channels - 1)
        ...
        while loop(active):
            ...
            active_medium = active & dr.neq(
                medium, None)  # TODO this is not necessary
            ...
            with dr.resume_grad(when=not is_primal):
                ...
                tr, free_flight_pdf = medium.eval_tr_and_pdf(
                ...
                if self.handle_null_scattering:
                    scatter_prob = index_spectrum(
                        mei.sigma_t, channel) / index_spectrum(mei.combined_extinction, channel)
                    act_null_scatter = (sampler.next_1d(
                        active_medium) >= scatter_prob) & active_medium
                    act_medium_scatter = ~act_null_scatter & active_medium
                    weight[act_null_scatter] *= mei.sigma_n / \
                        dr.detach(1 - scatter_prob)
                    # weight[act_null_scatter] *= mei.sigma_n / dr.detach(1 - scatter_prob)
                    weight[act_null_scatter] *= dr.select(dr.neq(dr.detach(1 - scatter_prob), 0.0),
                                                          mei.sigma_n /
                                                          dr.detach(
                                                              1 - scatter_prob),
                                                          0.0)
                ...
                weight[act_medium_scatter] *= dr.select(dr.neq(dr.detach(scatter_prob), 0.0),
                                                        mei.sigma_s /
                                                        dr.detach(
                                                            scatter_prob),
                                                        0.0)
                ...
                # Add transient distance given by medium sampling/intersection inside
                # distance[act_medium_scatter] = distance + last_scatter_event.t * η    # Homogeneous media

                mei = dr.detach(mei)
                if not is_primal and dr.grad_enabled(weight):
                    Lo = dr.detach(
                        dr.select(active_medium | escaped_medium, L / dr.max(1e-8, weight), 0.0))
                ...
                with dr.suspend_grad():
                    wo, phase_pdf = phase.sample(phase_ctx, mei, sampler.next_1d(
                ...
                # --------------------- Emitter sampling ---------------------
                if self.use_nee:
                    active_e_surface = active_surface & mi.has_flag(
                        bsdf.flags(), mi.BSDFFlags.Smooth) & (depth + 1 < self.max_depth)
                    ...
                    emitted, ds = self.sample_emitter(
                        ref_interaction, scene, sampler, medium, channel, active_e, mode=dr.ADMode.Primal)
                    # Query the BSDF for that emitter-sampled direction
                    bsdf_val, bsdf_pdf = bsdf.eval_pdf(
                        ctx, si, si.to_local(ds.d), active_e_surface)
                    phase_val = phase.eval(
                        phase_ctx, mei, ds.d, active_e_medium)
                    nee_weight = dr.select(
                        active_e_surface, bsdf_val, phase_val)
                    nee_directional_pdf = dr.select(ds.delta, 0.0, dr.select(
                        active_e_surface, bsdf_pdf, phase_val))
                ...
                if not is_primal and dr.grad_enabled(bsdf_eval):
                    Lo = bsdf_eval * \
                        dr.detach(
                            dr.select(active, L / dr.max(1e-8, bsdf_eval), 0.0))
    ...
    def sample_emitter(self, ref_interaction, scene, sampler, medium, channel,
        ...
        while loop(active):
            remaining_dist = ds.dist * \
                (1.0 - mi.math.ShadowEpsilon) - total_dist
            ...
            if not is_primal and dr.grad_enabled(tr_multiplier):
                active_adj = (active_surface | active_medium) & (
                    tr_multiplier > 0.0)
                dr.backward(tr_multiplier * dr.detach(dr.select(active_adj,
                            δL * adj_emitted / tr_multiplier, 0.0)))

            # transmittance *= dr.detach(tr_multiplier)
            transmittance *= dr.select(dr.isfinite(dr.detach(tr_multiplier)),
                                       dr.detach(tr_multiplier), 0.0)
