"""
    Polarized version of "mitransient/visualization.py"
"""
...
def show_video_polarized(stokes, degree_of_polarization, angle_of_polarization, type_of_polarization, chirality, save_path=None, 
    ...
    logging.getLogger('matplotlib').setLevel(logging.ERROR) # Supress matplotlib warning
    
    stokes_np = np.array(stokes)
    timebins = stokes.shape[-2]

    # Normalize Stokes vector with the intensity component
    stokes_np[..., 1:] = stokes_np[..., 1:] / np.maximum(stokes_np[..., [0]], 0.0001)
    stokes12_max = np.maximum(np.abs(np.max(stokes_np[..., 1:3], axis=None)), np.abs(np.min(stokes_np[..., 1:3])))
    stokes3_max = np.maximum(np.abs(np.max(stokes_np[..., 3], axis=None)), np.abs(np.min(stokes_np[..., 3])))
    ...
    def plot_polarimetric_frame(timebin, stokes, degree_of_polarization, angle_of_polarization, type_of_polarization, chirality, show_false_color):
        images = list()
        # Plot Stokes vector
        plt.subplot(nrows, 4, 1); images.append(plt.imshow(stokes_np[:, :, timebin, 0], cmap='Greys_r', vmin=0.0, vmax=stokes0_max)); plt.title('s0')
        plt.subplot(nrows, 4, 2); images.append(plt.imshow(stokes_np[:, :, timebin, 1], cmap='seismic', vmin=-stokes12_max, vmax=stokes12_max)); plt.title('s1')
        plt.subplot(nrows, 4, 3); images.append(plt.imshow(stokes_np[:, :, timebin, 2], cmap='seismic', vmin=-stokes12_max, vmax=stokes12_max)); cbar_stokes12 = plt.colorbar(fraction=0.046, pad=0.04); plt.title('s2'); plt.axis('off')
        plt.subplot(nrows, 4, 4); images.append(plt.imshow(stokes_np[:, :, timebin, 3], cmap='seismic', vmin=-stokes3_max, vmax=stokes3_max)); cbar_stokes3 = plt.colorbar(fraction=0.046, pad=0.04); plt.title('s3'); plt.axis('off')

        cbar_stokes12.ax.tick_params(labelsize=7); cbar_stokes3.ax.tick_params(labelsize=7)
        
        # Plot false color images
        if show_false_color:
            plt.subplot(nrows, 4, 5); images.append(plt.imshow(degree_of_polarization[:, :, timebin, :])); plt.title('Degree of polarization')
            plt.subplot(nrows, 4, 6); images.append(plt.imshow(angle_of_polarization[:, :, timebin, :])); plt.title('Angle of polarization')
            plt.subplot(nrows, 4, 7); images.append(plt.imshow(type_of_polarization[:, :, timebin, :])); plt.title('Type of polarization')
            plt.subplot(nrows, 4, 8); images.append(plt.imshow(chirality[:, :, timebin, :])); plt.title('Chirality')
    ...
    if display_method == DisplayMethod.ShowVideo:
        ...
        images = plot_polarimetric_frame(0, stokes, degree_of_polarization, angle_of_polarization, type_of_polarization, chirality, show_false_color)
    ...
    ### Save video to file
        ...
        for timebin in range(timebins):
            plot_polarimetric_frame_update(timebin, images, stokes, degree_of_polarization, angle_of_polarization, type_of_polarization, chirality, show_false_color)
    ...
    ### Save frames to separate files
        ...
        for timebin in range(timebins):
            plot_polarimetric_frame_update(timebin, images, stokes, degree_of_polarization, angle_of_polarization, type_of_polarization, chirality, show_false_color)
            plt.savefig(f'{save_path}/{timebin:03d}.png', bbox_inches='tight', pad_inches=0)
...
def degree_of_polarization(stokes, s0_minimum=0.01):
    ...
    """
    dop = np.sqrt(np.square(stokes[..., 1]) + np.square(stokes[..., 2]) + np.square(stokes[..., 3])) / np.maximum(stokes[..., 0], s0_minimum)
...
def degree_of_linear_polarization(stokes, s0_minimum=0.01):
    ...
    :param s0_minimum: Minimum value of the s0 component, to avoid NaNs caused by dividing by 0. Default value of 0.1.
    """
    dop = np.sqrt(np.square(stokes[..., 1]) + np.square(stokes[..., 2])) / np.maximum(stokes[..., 0], s0_minimum)
    dop = np.clip(dop, a_min=0.0, a_max=1.0)
    return dop
...
def degree_of_circular_polarization(stokes, s0_minimum=0.01):
    ...
    :param s0_minimum: Minimum value of the s0 component, to avoid NaNs caused by dividing by 0. Default value of 0.1.
    """
    dop = np.sqrt(np.square(stokes[..., 3])) / np.maximum(stokes[..., 0], s0_minimum)
    dop = np.clip(dop, a_min=0.0, a_max=1.0)
    return dop
...
def polarization_generate_false_color(stokes, aolp_intensity_scaling=1.0, s0_minimum=0.01):
    ...
    aolp_rainbow_scaled = aolp_rainbow * aolp_intensity_scaling * clone_tensor(dop_linear, 3)
    ...
    type_of_polarization = clone_tensor(dop_linear, 3) * linear_polarization_color + clone_tensor(dop_circular, 3) * circular_polarization_color
